...
impl Value {
    ...
    fn cap_width_cold(width: u32) -> u32 {
        ...
        if !WARNED.swap(true, Ordering::Relaxed) {
            eprintln!(
                "[xezim][warning] value width {} exceeds cap {}; clamping \
    ...
    fn fill_at(&self, width: u32) -> Value {
        ...
        if width <= 64 {
            ...
            Value {
                storage: wide_filled_bits(width, bit),
                width,
                is_signed: false,
                is_real: false,
                is_fill: false,
    ...
    fn fill_pair_cold(&self, other: &Value) -> (Value, Value) {
        ...
        let a = if self.is_fill {
            self.fill_at(w)
        } else {
            self.clone()
        };
        let b = if other.is_fill {
            other.fill_at(w)
        } else {
            other.clone()
        };
    ...
    pub fn zero(width: u32) -> Self {
        ...
        if width <= 64 {
            ...
            Self {
                storage: ValueStorage::Wide(Box::new(WidePlanes::filled(width, LogicBit::Zero))),
                width,
                is_signed: false,
                is_real: false,
                is_fill: false,
    ...
    pub fn to_u128(&self) -> u128 {
        match &self.storage {
            ...
            ValueStorage::Wide(bits) => {
                let w0 =
                    bits.val.first().copied().unwrap_or(0) & !bits.xz.first().copied().unwrap_or(0);
                let w1 =
                    bits.val.get(1).copied().unwrap_or(0) & !bits.xz.get(1).copied().unwrap_or(0);
    ...
    pub fn set_inline_planes(&mut self, v: u64, x: u64) {
        self.storage = ValueStorage::Inline {
            val_bits: v,
            xz_bits: x,
        };
    }
        ...
        match &self.storage {
            ...
            _ => {
                ...
                let mut result = if self.is_signed {
                    ...
                    let fill = if sign == LogicBit::One {
                        LogicBit::One
                    } else {
                        LogicBit::Zero
                    };
                    Value {
                        storage: if target <= 64 {
                            let fill_val = if fill == LogicBit::One {
                                Self::mask(target)
                            } else {
                                0
                            };
    ...
    pub fn resize_for_assign(&self, target: u32) -> Value {
        ...
        if !self.is_fill
            && (!self.is_signed || target <= self.width || self.width == 0 || self.is_real || {
    ...
    pub fn power(&self, other: &Value) -> Value {
        ...
        let result: u64 = if neg_exp {
            match self.to_i64().unwrap_or(0) {
                ...
                -1 => {
                    if other.to_i64().unwrap_or(0) % 2 == 0 {
                        1
                    } else {
                        u64::MAX
                    }
    ...
    fn bitwise_and_slow(&self, other: &Value) -> Value {
        ...
        if let Some((a, b, w)) = self.wide_bitwise_pair(other) {
            ...
            let bits = Self::wide_bitwise_lanes(
                a,
                b,
                w,
                |av, ax, bv, bx| {
    ...
    pub fn bitwise_or(&self, other: &Value) -> Value {
        if !self.is_fill && !other.is_fill {
            ...
            ) = (&self.storage, &other.storage)
            {
                ...
                return Value {
                    storage: ValueStorage::Inline {
                        val_bits: result_val | ((av & !ax) | (bv & !bx)),
                        xz_bits: result_xz & Self::mask(w),
                    },
                    width: w,
                    is_signed: false,
    ...
    pub fn bitwise_not(&self) -> Value {
        match &self.storage {
            ValueStorage::Inline { val_bits, xz_bits } => {
                ...
                Value {
                    storage: ValueStorage::Inline {
                        val_bits: (!val_bits & !xz_bits) & mask,
                        xz_bits: *xz_bits,
                    },
                    width: self.width,
                    is_signed: self.is_signed,
            ...
            ValueStorage::Wide(bits) => {
                let new_bits: Vec<LogicBit> = bits
                    .iter()
                    .map(|b| match b {
                    ...
                    .collect();
                Value {
                    storage: ValueStorage::Wide(Box::new(WidePlanes::from_bits(&new_bits))),
                    width: self.width,
                    is_signed: self.is_signed,
                    is_real: false,
                    is_fill: false,
        ...
        match (&self.storage, &other.storage) {
            ...
            _ => {
                ...
                for i in 0..w as usize {
                    let a = if i < self.width as usize {
                        self.get_bit(i)
                    } else {
                        LogicBit::Zero
                    };
                    let b = if i < other.width as usize {
                        other.get_bit(i)
                    } else {
                        LogicBit::Zero
                    };
    ...
    pub fn shift_left(&self, amount: &Value) -> Value {
        ...
        match &self.storage {
            ...
            _ => {
                ...
                // IEEE 1800 §11.4.10: move both planes and zero-fill vacated bits.
                result.copy_bits_from(
                    amt as usize,
                    self,
                    0,
    ...
    pub fn is_equal(&self, other: &Value) -> Value {
        ...
        if self.has_xz() || other.has_xz() {
            ...
            let top_a = if self.width > 0 {
                self.get_bit((self.width - 1) as usize)
            } else {
                LogicBit::Zero
            };
            let top_b = if other.width > 0 {
                other.get_bit((other.width - 1) as usize)
            } else {
                LogicBit::Zero
            };
            for i in 0..w {
    ...
    fn casex_eq_slow(&self, other: &Value) -> Value {
        ...
        for i in 0..w {
            ...
            if matches!(a, LogicBit::X | LogicBit::Z) || matches!(b, LogicBit::X | LogicBit::Z) {
                continue;
            }
            if a != b {
                return Value::from_u64(0, 1);
    ...
    pub fn wildcard_eq(&self, other: &Value) -> Value {
        ...
        let top_l = if self.width > 0 {
            self.get_bit((self.width - 1) as usize)
        } else {
            LogicBit::Zero
        };
        let top_r = if other.width > 0 {
            other.get_bit((other.width - 1) as usize)
        } else {
            LogicBit::Zero
        };
        ...
        for i in 0..w {
            let l = if i < self.width as usize {
                self.get_bit(i)
            } else if sign_l {
                top_l
            } else {
            ...
            let r = if i < other.width as usize {
                other.get_bit(i)
            } else if sign_r {
                top_r
            } else {
    ...
    pub fn less_equal(&self, other: &Value) -> Value {
        ...
        if self.is_signed && other.is_signed {
            Value::from_u64(
                (self.to_i64().unwrap_or(0) <= other.to_i64().unwrap_or(0)) as u64,
                1,
            )
        } else {
            Value::from_u64(
                (self.to_u64().unwrap_or(0) <= other.to_u64().unwrap_or(0)) as u64,
                1,
            )
        }
    }
        ...
        match &self.storage {
            ...
            ValueStorage::Wide(bits) => {
                if bits.contains(&LogicBit::One) {
                    Some(true)
                } else if bits.iter().any(|b| matches!(b, LogicBit::X | LogicBit::Z)) {
                    None
                } else {
                    Some(false)
                }
    ...
    pub fn reduce_and(&self) -> Value {
        ...
        match &self.storage {
            ValueStorage::Inline { val_bits, xz_bits } => {
                ...
                if (!*val_bits & !*xz_bits & mask) != 0 {
                    Value::from_u64(0, 1)
                } else if *xz_bits & mask != 0 {
                    Value::new(1)
                } else {
                    Value::from_u64(1, 1)
                }
    ...
    pub fn to_string(&self) -> String {
        ...
        for b in 0..bytes {
            ...
            for bit in 0..8 {
                if self.get_bit((b * 8 + bit) as usize) == LogicBit::One {
                    byte_val |= 1 << bit;
                }
            }
            if byte_val == 0 {
...
impl<'a> BitsRef<'a> {
    pub fn first(&self) -> Option<LogicBit> {
        if self.value.width > 0 {
            Some(self.value.get_bit(0))
        } else {
            None
        }
    }
    ...
    pub fn get(&self, i: usize) -> Option<LogicBit> {
        if (i as u32) < self.value.width {
            Some(self.value.get_bit(i))
        } else {
            None
        }
    }
...
impl<'a> PartialEq for BitsRef<'a> {
    fn eq(&self, other: &Self) -> bool {
        if self.value.width != other.value.width {
            return false;
        }
        for i in 0..self.value.width as usize {
            if self.value.get_bit(i) != other.value.get_bit(i) {
                return false;
            }
        }
        true
...
mod tests {
    ...
    fn wide_unknown_merge_matches_scalar_reference() {
        ...
        for width in [65, 127, 128, 129, 292, 512, 4097] {
            let mut known = Value::zero(width);
            for i in 0..width as usize {
                known.set_bit(i, LogicBit::One);
            }
            let identical = known.merge_unknown(&known);
            assert!(!identical.has_xz(), "padding must not contain unknown bits");
            assert!((0..width as usize).all(|i| identical.get_bit(i) == LogicBit::One));
            for other_width in [32, width, width + 7] {
                let mut a = Value::zero(width);
                let mut b = Value::zero(other_width);
                for i in 0..width as usize {
                    a.set_bit(i, states[i % 4]);
                }
                for i in 0..other_width as usize {
                    b.set_bit(i, states[(i / 4) % 4]);
                }
                let merged = a.merge_unknown(&b);
                assert_eq!(merged.width, width.max(other_width));
                for i in 0..merged.width as usize {
                    let expected = match (a.get_bit(i), b.get_bit(i)) {
                        (LogicBit::Zero, LogicBit::Zero) => LogicBit::Zero,
                        (LogicBit::One, LogicBit::One) => LogicBit::One,
                        _ => LogicBit::X,
                    };
                    assert_eq!(
                        merged.get_bit(i),
                        expected,
                        "width={width} other={other_width} bit={i}"
                    );
                }
            }
        }
    ...
    #[test]
    fn wide_logical_shifts_match_scalar_reference() {
        use super::*;
        for width in [65, 127, 128, 129, 292, 512, 4097] {
            let mut input = Value::zero(width);
            for i in 0..width as usize {
                input.set_bit(
                    i,
                    [LogicBit::Zero, LogicBit::One, LogicBit::X, LogicBit::Z][i % 4],
                );
            }
            for amount in [
                0,
                ...
                let shift = Value::from_u64(amount as u64, 32);
                let left = input.shift_left(&shift);
                let right = input.shift_right(&shift);
                for i in 0..width as usize {
                    let l = i
                        .checked_sub(amount as usize)
                        .map(|s| input.get_bit(s))
                        .unwrap_or(LogicBit::Zero);
                    let r = if (i as u64 + amount as u64) < width as u64 {
                        input.get_bit(i + amount as usize)
                    } else {
                        LogicBit::Zero
                    };
                    assert_eq!(
                        left.get_bit(i),
                        l,
                        "left width={width} amount={amount} bit={i}"
                    );
                    assert_eq!(
                        right.get_bit(i),
                        r,
                        "right width={width} amount={amount} bit={i}"
                    );
                }
            }
            for unknown in [LogicBit::X, LogicBit::Z] {
                let mut shift = Value::zero(32);
                shift.set_bit(0, unknown);
                for result in [input.shift_left(&shift), input.shift_right(&shift)] {
                    assert!((0..width as usize).all(|i| result.get_bit(i) == LogicBit::X));
                }
            }
        }
    }
    ...
    fn unsized_decimal_literal_wrap_is_reported_at_the_2_31_boundary() {
        ...
        assert_eq!(
            Value::unsized_decimal_wrap(Some(64), 10, "3000000000"),
            None
        );
    ...
    fn contiguous_bit_copy_reports_changes() {
        ...
        for offset in 0..128 {
            assert_eq!(
                wide_target.get_bit(32 + offset),
                wide_source.get_bit(offset)
            );
        }
    ...
    #[test]
    fn signed_range_select_handles_wide_partial_overlap() {
        let source = patterned(130, 7, false);
        let got = source.range_select_signed(159, -30);
        assert_eq!(got.width, 190);
        for bit in 0..190usize {
            let src = bit as i64 - 30;
            let expected = if (0..130).contains(&src) {
                source.get_bit(src as usize)
            } else {
                LogicBit::X
            };
            assert_eq!(got.get_bit(bit), expected, "result bit {bit}");
        }
    }

    #[test]
    fn signed_range_select_bounds_absurd_dynamic_span() {
        let source = patterned(130, 11, false);
        let got = source.range_select_signed(i64::MAX, i64::MIN + 1);
        assert_eq!(got.width, source.width);
        assert!((0..got.width as usize).all(|bit| got.get_bit(bit) == LogicBit::X));
    }
    ...
    fn case_wildcard_reference(a: &Value, b: &Value, casex: bool) -> Value {
        ...
        for i in 0..w {
            let x = if i < a.width as usize {
                a.get_bit(i)
            } else {
                ext_a
            };
            let y = if i < b.width as usize {
                b.get_bit(i)
            } else {
                ext_b
            };
    ...
    fn split_bitwise_and_assign_resize_match_reference() {
                        ...
                        for sa in [false, true] {
                            for sb in [false, true] {
                                ...
                                for (got, want, nm) in [
                                    ...
                                    assert_eq!(got.width, want.width, "{nm} width {a} vs {b}");
                                    for i in 0..want.width as usize {
                                        assert_eq!(
                                            got.get_bit(i),
                                            want.get_bit(i),
        ...
        for seed in 0..4u64 {
            ...
            let bits_equal =
                a.width == b.width && (0..a.width as usize).all(|i| a.get_bit(i) == b.get_bit(i));
    ...
    fn test_decimal_wide_and_malformed() {
        ...
        assert_eq!(
            d("340282366920938463463374607431768211456", 128),
            "0".repeat(32)
        );
        assert_eq!(
            d("340282366920938463463374607431768211457", 128),
                ...
                for left_signed in [false, true] {
                    for right_signed in [false, true] {
                        ...
                        for left_code in 0..left_count {
                            let left = four_state_value(left_code, left_width, left_signed);
                            for right_code in 0..right_count {
                                let right = four_state_value(right_code, right_width, right_signed);
                                assert_eq!(
    ...
    fn test_logic_equiv() {
        ...
        assert_eq!(
            Value::from_u64(2, 4).logic_equiv(&o).get_bit(0),
            LogicBit::One
        );
    }
    ...
    fn test_wildcard_eq_ne() {
        // 4'b1010 ==? 4'b1010 = 1
        assert_eq!(
            Value::from_u64(0b1010, 4)
                .wildcard_eq(&Value::from_u64(0b1010, 4))
                .get_bit(0),
            LogicBit::One
        );
        // 4'b1010 ==? 4'b1011 = 0
        assert_eq!(
            Value::from_u64(0b1010, 4)
                .wildcard_eq(&Value::from_u64(0b1011, 4))
                .get_bit(0),
            LogicBit::Zero
        );
        ...
        assert_eq!(
            Value::from_u64(0b1011, 4).wildcard_eq(&rhs).get_bit(0),
            LogicBit::One
        );
        // 4'b0011 ==? 4'b1x1x = 0  (bit3: 0 vs 1, hard mismatch)
        assert_eq!(
            Value::from_u64(0b0011, 4).wildcard_eq(&rhs).get_bit(0),
            LogicBit::Zero
        );
        ...
        assert_eq!(
            lhs.wildcard_eq(&Value::from_u64(0b1010, 4)).get_bit(0),
            LogicBit::X
        );
        // !=? is the inverse; x stays x
        assert_eq!(
            Value::from_u64(0b1010, 4)
                .wildcard_ne(&Value::from_u64(0b1011, 4))
                .get_bit(0),
            LogicBit::One
        );
        assert_eq!(
            Value::from_u64(0b1011, 4).wildcard_ne(&rhs).get_bit(0),
            LogicBit::Zero
        );
        assert_eq!(
            lhs.wildcard_ne(&Value::from_u64(0b1010, 4)).get_bit(0),
            LogicBit::X
        );
    }
    ...
    fn test_is_nonzero_definite_one() {
        ...
        assert_eq!(
            v3.logic_and(&Value::from_u64(1, 1)).get_bit(0),
            LogicBit::One
        );
...
impl Value {
    ...
    pub fn range_select_signed(&self, hi: i64, lo: i64) -> Value {
        ...
        // Dynamic indices can underflow before reaching this helper. Keep the
        // result bounded just like the unsigned slow path; otherwise a single
        // malformed select can walk billions of positions even though Value's
        // constructor has already clamped its allocation.
        let width = if span > Self::MAX_WIDTH as i128 {
            self.width.max(1) as usize
        } else {
            span as usize
        };
        if self.is_fill {
            return self.fill_at(width as u32);
        }

        let source_width = self.width as i64;
        if lo >= 0 && hi < source_width {
            return self.range_select(hi as usize, lo as usize);
        }

        let mut result = Value::new(width as u32); // out-of-range bits stay x
        let valid_lo = lo.max(0);
        let valid_hi = hi.min(source_width - 1);
        if valid_lo > valid_hi {
            return result;
        }

        let dst_start = (valid_lo as i128 - lo as i128) as usize;
        let valid_width = (valid_hi - valid_lo + 1) as usize;
        let src_start = valid_lo as usize;
